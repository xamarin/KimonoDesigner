...
namespace KimonoCore
{
	...
	public class KimonoShapeOval : KimonoShape, IKimonoCodeGeneration, IKimonoPropertyConsumer
	{
		...
		public override string ToSkiaSharpPath()
		{
			...
			// Define path
			sourceCode += $"{pathName}.AddOval(new SKRect({Rect.Left}f ,{Rect.Top}f, {Rect.Right}f, {Rect.Bottom}f), SKPathDirection.Clockwise);\n";
		...
		public override string ToKimonoCore()
		{
			...
			sourceCode += $"// Draw {Name} shape\n" +
				$"var {ElementName} = new KimonoShapeOval({Left}f, {Top}f, {Right}f, {Bottom}f)" + "{" +
